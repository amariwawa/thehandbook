...
export default function SidebarLayout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <div className="flex min-h-screen bg-[#f8fafb] dark:bg-[#000000]">
      {/* Sidebar */}
      <aside className="!hidden lg:!flex w-72 bg-white dark:bg-black border-r border-slate-100 dark:border-white/10 flex-col fixed h-full z-20 transition-colors">
          ...
          {navItems.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                href={item.href}
                className={`group flex items-center gap-4 px-4 py-3.5 rounded-2xl font-bold transition-all duration-300 ${
                  isActive
      ...
      {/* Main Content */}
      <main className="flex-1 ml-0 lg:ml-72 transition-colors">
