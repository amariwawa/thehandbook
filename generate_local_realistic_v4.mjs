...
// Common structural keys that should NOT become topics
const STRUCTURAL_KEYS = new Set([
  'title', 'topic', 'section', 'part', 'part_number', 's_no',
  'description', 'content', 'contents', 'notes', 'objectives',
  'focus', 'period', 'syllabus_title', 'exam', 'subject',
  'detailed_syllabus', 'syllabus', 'main_sections', 'sections',
  'parts', 'theory_sections', 'practical_section', 'topics', 'subtopics'
]);
...
function extractTopicsSmart(obj, sectionName = 'General', topics = [], depth = 0) {
  if (typeof obj === 'string') {
    // String leaf node - skip if it looks like a structural field
    return;
  }
  ...
  if (Array.isArray(obj)) {
    for (const item of obj) {
      if (typeof item === 'string' && item.length > 3) {
        // This is actual content (subtopic, note, etc.)
        topics.push({ section: sectionName, topic: item, content: '', objectives: [] });
      } else if (typeof item === 'object' && item !== null) {
        // It's an object in an array - look for meaningful fields
        ...
        if (title && typeof title === 'string' && !isStructuralKey(title)) {
          // Good topic found
          const content = (typeof contents === 'string' ? contents : '') +
                          (notesStr ? ' ' + notesStr : '');
          topics.push({ section: sectionName, topic: title, content, objectives: [] });

          // Also add subtopics
          if (Array.isArray(subtopics)) {
            for (const st of subtopics) {
              if (typeof st === 'string' && st.length > 3) {
                topics.push({ section: sectionName, topic: `${title}: ${st}`, content: '', objectives: [] });
              } else if (typeof st === 'object' && st !== null && st.title) {
                topics.push({ section: sectionName, topic: `${title}: ${st.title}`, content: '', objectives: [] });
              }
            }
          }
          // Also add contents array items
          if (Array.isArray(contents)) {
            for (const c of contents) {
              if (typeof c === 'string' && c.length > 3) {
                topics.push({ section: sectionName, topic: `${title}: ${c}`, content: '', objectives: [] });
              }
            }
          }
          // Also add notes array items
          if (Array.isArray(notes)) {
            for (const n of notes) {
              if (typeof n === 'string' && n.length > 3) {
                topics.push({ section: sectionName, topic: `${title}: ${n}`, content: '', objectives: [] });
              }
            }
          }
        } else {
          // No good title - recurse
          extractTopicsSmart(item, sectionName, topics, depth + 1);
        }
      }
    }
    ...
    for (const key of Object.keys(obj)) {
      ...
      // Determine section name from parent key if meaningful
      let newSection = sectionName;
      if (!isStructuralKey(key) && key.length > 3 && !key.match(/^\d+$/)) {
        newSection = key.replace(/_/g, ' ').replace(/\s+/g, ' ').trim();
        // Capitalize first letter
        newSection = newSection.charAt(0).toUpperCase() + newSection.slice(1);
      }

      if (isStructuralKey(key)) {
        // These keys hold actual content arrays/objects
        if (Array.isArray(val)) {
          extractTopicsSmart(val, sectionName, topics, depth + 1);
        } else if (typeof val === 'object' && val !== null) {
          extractTopicsSmart(val, sectionName, topics, depth + 1);
        }
      } else if (Array.isArray(val)) {
        // Non-structural key with array value - could be topic list
        for (const item of val) {
          if (typeof item === 'string' && item.length > 3) {
            topics.push({ section: newSection, topic: item, content: '', objectives: [] });
          } else if (typeof item === 'object' && item !== null) {
            extractTopicsSmart(item, newSection, topics, depth + 1);
          }
        }
      } else if (typeof val === 'string' && val.length > 10 && val.length < 500) {
        // Non-structural key with descriptive string value
        const topicName = key.replace(/_/g, ' ').replace(/^\d+\s*/, '').trim();
        if (topicName.length > 3 && !isStructuralKey(topicName)) {
          topics.push({ section: sectionName, topic: topicName, content: val, objectives: [] });
        }
      } else if (typeof val === 'object' && val !== null) {
        // Check if the object itself has title/topic
        const objTitle = val.title || val.topic || '';
        if (objTitle && typeof objTitle === 'string' && !isStructuralKey(objTitle)) {
          const content = val.content || val.notes || val.description || '';
          topics.push({ section: newSection, topic: objTitle, content: typeof content === 'string' ? content : '', objectives: [] });

          // Also check for subtopics/contents inside
          if (val.subtopics && Array.isArray(val.subtopics)) {
            for (const st of val.subtopics) {
              if (typeof st === 'string' && st.length > 3) {
                topics.push({ section: newSection, topic: `${objTitle}: ${st}`, content: '', objectives: [] });
              }
            }
          }
          if (val.contents && Array.isArray(val.contents)) {
            for (const c of val.contents) {
              if (typeof c === 'string' && c.length > 3) {
                topics.push({ section: newSection, topic: `${objTitle}: ${c}`, content: '', objectives: [] });
              }
            }
          }
          if (val.notes && Array.isArray(val.notes)) {
            for (const n of val.notes) {
              if (typeof n === 'string' && n.length > 3) {
                topics.push({ section: newSection, topic: `${objTitle}: ${n}`, content: '', objectives: [] });
              }
            }
          }
        } else {
          extractTopicsSmart(val, newSection, topics, depth + 1);
        }
      }
    }
  }
...
function extractTopics(syllabus) {
  const topics = [];
  extractTopicsSmart(syllabus, 'General', topics, 0);
  ...
  for (const t of topics) {
    ...
    // Filter out topics that are just structural keys or too short
    if (!seen.has(key) && t.topic && t.topic.length > 5 && !isStructuralKey(t.topic)) {
      seen.add(key);
      unique.push(t);
    }
  }

  // If we got very few topics, fall back to extracting all string leaves
  if (unique.length < 3) {
    const allStrings = [];
    function collectStrings(o, sec = 'General') {
      if (typeof o === 'string' && o.length > 10 && o.length < 400) {
        allStrings.push({ section: sec, topic: o.slice(0, 120), content: o, objectives: [] });
      } else if (Array.isArray(o)) {
        o.forEach(i => collectStrings(i, sec));
      } else if (typeof o === 'object' && o !== null) {
        for (const k of Object.keys(o)) {
          const v = o[k];
          const newSec = (!isStructuralKey(k) && k.length > 3) ? k.replace(/_/g, ' ') : sec;
          if (typeof v === 'string') collectStrings(v, newSec);
          else if (Array.isArray(v)) v.forEach(i => collectStrings(i, newSec));
          else if (typeof v === 'object') collectStrings(v, newSec);
        }
      }
    }
    collectStrings(syllabus);
    // Deduplicate fallback
    const seen2 = new Set();
    for (const t of allStrings) {
      const key = t.topic.toLowerCase().slice(0, 200);
      if (!seen2.has(key)) { seen2.add(key); unique.push(t); }
    }
  }
...
function buildScienceQuestion(topic, section, content, difficulty, subjectKey) {
  ...
  if (sk.includes('biology') || sk.includes('biolog')) {
    if (topicMatches(t, ['cell', 'organelle', 'cytoplasm', 'nucleus', 'mitochondria', 'ribosome', 'cell structure', 'cell component'])) {
    ...
    } else if (topicMatches(t, ['respiration', 'anaerobic', 'aerobic', 'glycolysis', 'gaseous exchange'])) {
    ...
    } else if (topicMatches(t, ['genetics', 'dna', 'gene', 'chromosome', 'allele', 'inheritance', 'mendel', 'monohybrid', 'dihybrid'])) {
    ...
    } else if (topicMatches(t, ['ecosystem', 'food chain', 'trophic', 'producer', 'consumer', 'decomposer', 'food web'])) {
    ...
    } else if (topicMatches(t, ['human', 'digestion', 'alimentary', 'enzyme', 'stomach', 'intestine', 'mammalian tooth'])) {
    ...
    } else if (topicMatches(t, ['circulatory', 'heart', 'blood', 'vessel', 'artery', 'vein', 'xylem', 'phloem'])) {
      if (topicMatches(t, ['xylem', 'phloem', 'vascular'])) {
    ...
    } else if (topicMatches(t, ['living organism', 'characteristic', 'level of organization', 'cell', 'tissue', 'organ', 'system'])) {
    ...
    } else if (topicMatches(t, ['monera', 'protista', 'fungi', 'plantae', 'animalia', 'bacteria', 'amoeba', 'euglena', 'rhizopus', 'fern'])) {
    ...
    } else if (topicMatches(t, ['reproduction', 'asexual', 'sexual', 'pollination', 'fertilization', 'contraception'])) {
    ...
    } else if (topicMatches(t, ['nutrition', 'autotrophic', 'heterotrophic', 'photosynthesis', 'chemosynthesis', 'food', 'class of food'])) {
    ...
    if (topicMatches(t, ['atom', 'electron', 'proton', 'neutron', 'shell', 'orbital', 'atomic structure'])) {
    ...
    } else if (topicMatches(t, ['periodic table', 'group', 'period', 'element', 'metal', 'non-metal', 'alkali', 'halogen'])) {
    ...
    } else if (topicMatches(t, ['chemical bond', 'ionic', 'covalent', 'metallic', 'electrovalent', 'hydrogen bond', 'van der waals'])) {
    ...
    } else if (topicMatches(t, ['acid', 'base', 'salt', 'ph', 'neutralization', 'titration', 'indicator'])) {
    ...
    } else if (topicMatches(t, ['organic', 'hydrocarbon', 'alkane', 'alkene', 'alkyne', 'alcohol', 'carbon compound'])) {
    ...
    } else if (topicMatches(t, ['mole', 'molar', 'avogadro', 'concentration', 'molality', 'molarity', 'molar mass'])) {
    ...
    } else if (topicMatches(t, ['oxidation', 'reduction', 'redox', 'electrolysis', 'electrochemical'])) {
    ...
    } else if (topicMatches(t, ['gas law', 'boyle', 'charles', 'pressure', 'volume', 'temperature', 'gay-lussac'])) {
    ...
    } else if (topicMatches(t, ['water', 'hard water', 'soft water', 'water treatment', 'water cycle'])) {
    ...
    if (topicMatches(t, ['force', 'newton', 'motion', 'velocity', 'acceleration', 'inertia', 'projectile'])) {
    ...
    } else if (topicMatches(t, ['energy', 'work', 'power', 'potential', 'kinetic', 'conservation of energy'])) {
    ...
    } else if (topicMatches(t, ['wave', 'sound', 'light', 'reflection', 'refraction', 'diffraction', 'interference'])) {
    ...
    } else if (topicMatches(t, ['electric', 'current', 'voltage', 'resistance', 'ohm', 'circuit', 'power', 'resistor', 'capacitor'])) {
    ...
    } else if (topicMatches(t, ['heat', 'temperature', 'specific heat', 'latent heat', 'thermometer', 'expansion', 'thermal'])) {
    ...
    } else if (topicMatches(t, ['matter', 'state of matter', 'solid', 'liquid', 'gas', 'kinetic theory', 'brownian'])) {
    ...
    } else if (topicMatches(t, ['position', 'distance', 'displacement', 'scalar', 'vector'])) {
    ...
    if (topicMatches(t, ['soil', 'texture', 'structure', 'profile', 'fertility', 'soil erosion'])) {
    ...
    } else if (topicMatches(t, ['pest', 'disease', 'control', 'pesticide', 'herbicide', 'fungicide'])) {
    ...
    } else if (topicMatches(t, ['crop', 'planting', 'harvest', 'monoculture', 'mixed farming', 'crop rotation'])) {
    ...
    } else if (topicMatches(t, ['animal', 'livestock', 'poultry', 'cattle', 'nutrition', 'feed', 'pasture'])) {
    ...
    } else if (topicMatches(t, ['agricultural', 'extension', 'finance', 'marketing', 'cooperative'])) {
...
function buildArtsQuestion(topic, section, content, difficulty, subjectKey) {
  ...
  if (sk.includes('government') || sk.includes('civic')) {
    ...
    if (topicMatches(t, ['creation', 'genesis', 'adam', 'eve', 'fall', 'eden'])) {
    ...
    } else if (topicMatches(t, ['jesus', 'miracle', 'parable', 'sermon', 'disciple', 'resurrection', 'ascension'])) {
    ...
    } else if (topicMatches(t, ['apostle', 'pentecost', 'holy spirit', 'gift', 'fruit', 'church'])) {
    ...
    } else if (topicMatches(t, ['jahiliyyah', 'idol', 'mecca', 'reform'])) {
    ...
    } else if (topicMatches(t, ['caliph', 'rashidun', 'abu bakr', 'umar', 'uthman', 'ali'])) {
    ...
    if (topicMatches(t, ['drama', 'play', 'tragedy', 'comedy', 'shakespeare', 'character', 'act', 'scene'])) {
    ...
    } else if (topicMatches(t, ['poetry', 'poem', 'stanza', 'verse', 'rhyme', 'metre', 'alliteration', 'assonance', 'sonnet'])) {
    ...
    } else if (topicMatches(t, ['prose', 'novel', 'short story', 'narrative', 'theme', 'plot', 'setting', 'conflict'])) {
    ...
    } else if (topicMatches(t, ['figurative', 'simile', 'metaphor', 'personification', 'hyperbole', 'imagery', 'symbol'])) {
    ...
    if (topicMatches(t, ['climate', 'weather', 'rainfall', 'temperature', 'wind', 'pressure', 'cloud'])) {
    ...
    } else if (topicMatches(t, ['rock', 'igneous', 'sedimentary', 'metamorphic', 'weathering', 'erosion', 'denudation'])) {
    ...
    } else if (topicMatches(t, ['population', 'migration', 'birth rate', 'death rate', 'census', 'demography'])) {
    ...
    } else if (topicMatches(t, ['map', 'scale', 'contour', 'relief', 'projection', 'grid', 'bearing'])) {
    ...
    } else if (topicMatches(t, ['vegetation', 'forest', 'savanna', 'desert', 'tundra', 'biome', 'rainforest'])) {
    ...
    } else if (topicMatches(t, ['latitude', 'longitude', 'time', 'timezone', 'gmt', 'equator', 'tropic'])) {
    ...
    if (topicMatches(t, ['demand', 'supply', 'equilibrium', 'price', 'market', 'elasticity'])) {
    ...
    } else if (topicMatches(t, ['money', 'bank', 'central bank', 'inflation', 'deflation', 'monetary policy', 'cbn'])) {
    ...
    } else if (topicMatches(t, ['tax', 'fiscal policy', 'budget', 'government revenue', 'expenditure', 'public finance'])) {
    ...
    } else if (topicMatches(t, ['advertising', 'promotion', 'sales', 'marketing', 'product', 'branding'])) {
    ...
    } else if (topicMatches(t, ['insurance', 'premium', 'policy', 'underwriter', 'indemnity', 'principle', 'pooling'])) {
    ...
    } else if (topicMatches(t, ['transport', 'communication', 'warehousing', 'banking', 'finance', 'auxiliary'])) {
    ...
    } else if (topicMatches(t, ['account', 'bookkeeping', 'ledger', 'journal', 'balance sheet', 'trial balance', 'profit', 'loss'])) {
    ...
    } else if (topicMatches(t, ['occupation', 'industry', 'production', 'tertiary', 'secondary', 'primary'])) {
    ...
    if (topicMatches(t, ['comprehension', 'passage', 'inference', 'summary', 'reading'])) {
    ...
    } else if (topicMatches(t, ['grammar', 'tense', 'parts of speech', 'clause', 'phrase', 'sentence', 'mood', 'voice'])) {
    ...
    } else if (topicMatches(t, ['lexis', 'synonym', 'antonym', 'homonym', 'vocabulary', 'register', 'jargon'])) {
    ...
    } else if (topicMatches(t, ['essay', 'writing', 'narrative', 'descriptive', 'argumentative', 'expository', 'letter'])) {
...
function buildVocationalQuestion(topic, section, content, difficulty, subjectKey) {
  ...
  if (sk.includes('computer') || sk.includes('ict') || sk.includes('data processing')) {
    if (topicMatches(t, ['hardware', 'cpu', 'memory', 'input', 'output', 'storage', 'peripheral', 'ram', 'rom'])) {
    ...
    } else if (topicMatches(t, ['software', 'operating system', 'application', 'program', 'algorithm', 'compiler', 'interpreter'])) {
    ...
    } else if (topicMatches(t, ['internet', 'network', 'www', 'email', 'protocol', 'domain', 'ip', 'browser'])) {
    ...
    } else if (topicMatches(t, ['programming', 'code', 'variable', 'loop', 'conditional', 'function', 'array', 'string'])) {
    ...
    } else if (topicMatches(t, ['database', 'dbms', 'sql', 'table', 'record', 'field', 'key'])) {
    ...
    if (topicMatches(t, ['nutrition', 'protein', 'carbohydrate', 'vitamin', 'mineral', 'diet', 'balanced diet'])) {
    ...
    } else if (topicMatches(t, ['sewing', 'pattern', 'fabric', 'textile', 'garment', 'stitch'])) {
    ...
    if (topicMatches(t, ['colour', 'pigment', 'primary', 'secondary', 'complementary', 'harmony', 'warm', 'cool'])) {
    ...
    } else if (topicMatches(t, ['perspective', 'drawing', 'proportion', 'scale', 'line', 'shape', 'form', 'texture'])) {
    ...
    } else if (topicMatches(t, ['music', 'note', 'scale', 'rhythm', 'melody', 'harmony', 'time signature', 'clef'])) {
    ...
    } else if (topicMatches(t, ['ceramic', 'clay', 'glaze', 'kiln', 'firing', 'pottery'])) {
    ...
    if (topicMatches(t, ['safety', 'precaution', 'ppe', 'hazard', 'first aid', 'workshop'])) {
    ...
    } else if (topicMatches(t, ['tool', 'equipment', 'machine', 'instrument', 'device', 'hand tool', 'power tool'])) {
    ...
    } else if (topicMatches(t, ['circuit', 'voltage', 'current', 'resistor', 'capacitor', 'transformer', 'diode', 'transistor'])) {
    ...
    } else if (topicMatches(t, ['welding', 'solder', 'braze', 'arc', 'gas', 'electrode'])) {
