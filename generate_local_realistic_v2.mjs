...
function buildArtsQuestion(topic, section, content, difficulty, subjectKey) {
  ...
  if (sk.includes('government') || sk.includes('civic')) {
    ...
    } else if (topicMatches(t, ['democracy', 'rule of law', 'separation of power', 'checks'])) {
...
// ========================
// SYLLABUS PARSER (ENHANCED)
// ========================

function extractTopics(syllabus) {
  const topics = [];

  // Structure 1: sections array with topics arrays
  if (syllabus.sections && Array.isArray(syllabus.sections)) {
    for (const section of syllabus.sections) {
      ...
      if (Array.isArray(sectionTopics)) {
        for (const topic of sectionTopics) {
          if (typeof topic === 'string') {
            topics.push({ section: sectionTitle, topic, content: '', objectives: [] });
          } else {
            topics.push({
      ...
      } else if (typeof sectionTopics === 'object' && sectionTopics !== null) {
        // topics is an object with arrays inside (e.g., literature)
        for (const key of Object.keys(sectionTopics)) {
          const val = sectionTopics[key];
          if (Array.isArray(val)) {
            for (const item of val) {
              topics.push({ section: sectionTitle, topic: `${key}: ${item}`, content: '', objectives: [] });
            }
            ...
            for (const subKey of Object.keys(val)) {
              ...
              if (Array.isArray(subVal)) {
                for (const item of subVal) {
                  topics.push({ section: sectionTitle, topic: `${key} → ${subKey}: ${item}`, content: '', objectives: [] });
                }
              }
            }
          }
      ...
      // Also handle objectives at section level
      if (section.objectives && Array.isArray(section.objectives)) {
        for (const obj of section.objectives) {
          topics.push({ section: sectionTitle, topic: obj, content: '', objectives: [] });
        }
      }
    }
  }

  // Structure 2: detailed_syllabus.sections (JAMB CRS style)
  if (syllabus.detailed_syllabus && syllabus.detailed_syllabus.sections && Array.isArray(syllabus.detailed_syllabus.sections)) {
    for (const section of syllabus.detailed_syllabus.sections) {
      const sectionTitle = section.section || section.title || 'General';
      const sectionTopics = section.topics || [];
      for (const topic of sectionTopics) {
        topics.push({
  ...
  // Structure 3: detailed_syllabus with nested objects (WAEC Agric style)
  if (syllabus.detailed_syllabus && !syllabus.detailed_syllabus.sections) {
    const ds = syllabus.detailed_syllabus;
    for (const sectionKey of Object.keys(ds)) {
      const sectionObj = ds[sectionKey];
      if (typeof sectionObj !== 'object' || sectionObj === null) continue;
      for (const topicKey of Object.keys(sectionObj)) {
        const topicObj = sectionObj[topicKey];
        if (typeof topicObj === 'string') {
          topics.push({ section: sectionKey, topic: topicKey, content: topicObj, objectives: [] });
        } else if (typeof topicObj === 'object' && topicObj !== null) {
          const contentParts = [];
          for (const subKey of Object.keys(topicObj)) {
            const subVal = topicObj[subKey];
            if (typeof subVal === 'string') contentParts.push(`${subKey}: ${subVal}`);
            else if (Array.isArray(subVal)) contentParts.push(`${subKey}: ${subVal.join('; ')}`);
          }
          topics.push({ section: sectionKey, topic: topicKey, content: contentParts.join('. '), objectives: [] });
        }
      }
    }
  }

  // Structure 4: top-level topics array
  ...
  // Structure 5: syllabus_sections
  if (syllabus.syllabus_sections && Array.isArray(syllabus.syllabus_sections)) {
  ...
  // Structure 6: paper1, paper2 with topics
  for (const key of Object.keys(syllabus)) {
    if (key.startsWith('paper') && syllabus[key].topics && Array.isArray(syllabus[key].topics)) {
      const paperTitle = syllabus[key].title || key;
      for (const topic of syllabus[key].topics) {
        topics.push({
          section: paperTitle,
          topic: topic.title || topic.topic || 'General',
          content: topic.content || '',
