...
// ========================
// RECURSIVE SYLLABUS PARSER
// ========================

function extractTopicsRecursive(obj, sectionName = 'General', topics = []) {
  if (typeof obj === 'string') {
    // A string leaf node - could be topic content
    return;
  }
  if (Array.isArray(obj)) {
    for (const item of obj) {
      if (typeof item === 'string') {
        ...
        // Extract title/topic from object, then recurse
        const title = item.title || item.topic || item.topic_number || item.part || item.section || '';
        if (title && typeof title === 'string' && title.length < 300) {
          topics.push({ section: sectionName, topic: title, content: item.content || item.notes || item.description || '', objectives: [] });
        }
        // Recurse into all properties
        for (const key of Object.keys(item)) {
          const val = item[key];
          if (key === 'subtopics' && Array.isArray(val)) {
            for (const st of val) {
              if (typeof st === 'string') {
                topics.push({ section: sectionName, topic: `${title}: ${st}`, content: '', objectives: [] });
              } else if (typeof st === 'object' && st !== null) {
                const stTitle = st.title || st.topic || JSON.stringify(st).slice(0, 200);
                topics.push({ section: sectionName, topic: stTitle, content: '', objectives: [] });
              }
            }
          } else if (key === 'topics' && Array.isArray(val)) {
            for (const t of val) {
              if (typeof t === 'string') {
                topics.push({ section: sectionName, topic: t, content: '', objectives: [] });
              } else {
                extractTopicsRecursive(t, sectionName, topics);
              }
            }
          } else if (key === 'content' && Array.isArray(val)) {
            for (const c of val) {
              if (typeof c === 'string') {
                topics.push({ section: sectionName, topic: c, content: '', objectives: [] });
              }
            }
          } else if (key === 'notes' && Array.isArray(val)) {
            for (const n of val) {
              if (typeof n === 'string') {
                topics.push({ section: sectionName, topic: n, content: '', objectives: [] });
              }
            }
          } else if (typeof val === 'object' && val !== null) {
            extractTopicsRecursive(val, title || sectionName, topics);
          }
        }
      }
    }
    ...
    for (const key of Object.keys(obj)) {
      const val = obj[key];
      const newSection = (key === 'detailed_syllabus' || key === 'syllabus_title' || key === 'exam' || key === 'subject') 
        ? sectionName 
        : (typeof val === 'string' && val.length < 300 ? val : key.replace(/_/g, ' '));

      if (key === 'detailed_syllabus' || key === 'syllabus' || key === 'sections' || key === 'parts' || key === 'main_sections' || key === 'theory_sections' || key === 'topics') {
        extractTopicsRecursive(val, sectionName, topics);
      } else if (typeof val === 'string' && val.length > 10 && key !== 'syllabus_title' && key !== 'exam' && key !== 'subject') {
        // A descriptive string value - this is likely a topic description
        const topicName = key.replace(/_/g, ' ').replace(/\d+\s*/g, '').trim();
        topics.push({ section: sectionName, topic: topicName, content: val, objectives: [] });
      } else if (Array.isArray(val)) {
        extractTopicsRecursive(val, newSection, topics);
      } else if (typeof val === 'object' && val !== null) {
        // Check if object has title/topic at this level
        const title = val.title || val.topic || '';
        if (title && typeof title === 'string') {
          topics.push({ section: sectionName, topic: title, content: val.content || val.notes || val.description || '', objectives: [] });
        }
        extractTopicsRecursive(val, newSection, topics);
      }
    }
...
function extractTopics(syllabus) {
  const topics = [];
  extractTopicsRecursive(syllabus, 'General', topics);
  // Deduplicate by topic name
  const seen = new Set();
  const unique = [];
  for (const t of topics) {
    const key = (t.section + '::' + t.topic).toLowerCase().slice(0, 300);
    if (!seen.has(key) && t.topic && t.topic.length > 2) {
      seen.add(key);
      unique.push(t);
    }
  }
